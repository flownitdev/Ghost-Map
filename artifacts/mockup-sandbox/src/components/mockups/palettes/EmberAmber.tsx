export function EmberAmber() {
  const accent = "#F59E0B";
  const accentDim = "rgba(245,158,11,0.15)";
  const accentBorder = "rgba(245,158,11,0.32)";
  const accentGlow = "rgba(245,158,11,0.18)";
  const bg = "#0f0d09";
  const surface = "rgba(17,14,9,0.92)";
  const primary = "#3d2a0a";
  ...
  return (
    <div className="relative w-full min-h-screen overflow-hidden" style={{ background: bg, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="absolute inset-0" style={{
        background: `radial-gradient(ellipse at 30% 50%, rgba(245,158,11,0.05) 0%, transparent 55%),
                     radial-gradient(ellipse at 80% 20%, rgba(251,191,36,0.03) 0%, transparent 40%),
                     linear-gradient(180deg, #0f0d09 0%, #131008 50%, #0e0c08 100%)`,
      }}>
        <svg className="absolute inset-0 w-full h-full opacity-[0.04]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="grid-ea" width="60" height="60" patternUnits="userSpaceOnUse">
              <path d="M 60 0 L 0 0 0 60" fill="none" stroke="#F59E0B" strokeWidth="0.5"/>
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#grid-ea)" />
        </svg>
        <svg className="absolute inset-0 w-full h-full opacity-[0.06]" xmlns="http://www.w3.org/2000/svg">
          <line x1="0" y1="280" x2="900" y2="260" stroke="#F59E0B" strokeWidth="3"/>
          <line x1="100" y1="0" x2="150" y2="620" stroke="#F59E0B" strokeWidth="2"/>
          <line x1="0" y1="420" x2="900" y2="410" stroke="#F59E0B" strokeWidth="1.5"/>
          <line x1="400" y1="0" x2="380" y2="620" stroke="#F59E0B" strokeWidth="2.5"/>
          <line x1="600" y1="0" x2="620" y2="620" stroke="#F59E0B" strokeWidth="1.5"/>
          <line x1="0" y1="170" x2="900" y2="180" stroke="#F59E0B" strokeWidth="1"/>
          <line x1="250" y1="0" x2="270" y2="620" stroke="#F59E0B" strokeWidth="1"/>
          <line x1="750" y1="0" x2="740" y2="620" stroke="#F59E0B" strokeWidth="1"/>
        </svg>
      </div>
        ...
        <div key={m.label} className="absolute flex flex-col items-center gap-1" style={{ left: m.x, top: m.y, transform: "translate(-50%,-50%)", zIndex: 10 }}>
          <div className="relative">
            <div className="w-3 h-3 rounded-full border-2" style={{
              background: m.risk === "high" ? "rgba(245,158,11,0.25)" : m.risk === "med" ? "rgba(245,158,11,0.12)" : "rgba(245,158,11,0.06)",
              borderColor: m.risk === "high" ? accent : m.risk === "med" ? "rgba(245,158,11,0.45)" : "rgba(245,158,11,0.2)",
          ...
          <span style={{ fontSize: "7px", letterSpacing: "0.1em", color: m.risk === "high" ? accent : "rgba(245,158,11,0.35)", fontFamily: "monospace" }}>{m.label}</span>
        </div>
      ))}
        ...
        <div className="flex items-center gap-1.5 flex-wrap justify-center">
          {categories.map((cat, i) => (
            <button key={cat} style={{
              ...
              background: i === 0 ? accentDim : "rgba(15,13,9,0.82)",
      ...
      <div className="absolute top-0 right-0 bottom-0 w-72 z-40 flex flex-col" style={{
        background: "linear-gradient(160deg, rgba(18,14,8,0.97) 0%, rgba(14,11,6,0.98) 100%)",
          ...
          {[
            ...
            <div key={loc.name} className="flex items-center gap-3 p-2.5 rounded-xl cursor-default" style={{
              ...
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{
                background: loc.risk === "high" ? accent : loc.risk === "med" ? "rgba(245,158,11,0.45)" : "rgba(245,158,11,0.2)",
              }} />
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-4 py-2 rounded-full" style={{
        ...
        <span style={{ fontSize: "10px", letterSpacing: "0.1em", color: "rgba(255,255,255,0.3)" }}>EMBER AMBER</span>
