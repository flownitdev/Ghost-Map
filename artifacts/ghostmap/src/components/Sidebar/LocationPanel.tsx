...
import { useExplorationLogs } from "@/hooks/useExplorationLogs";
import { useLocation } from "wouter";
import { useLocationAnalysis } from "@/hooks/useLocationAnalysis";
import { AIAnalysisPanel } from "./AIAnalysisPanel";
import { useNearby } from "@/hooks/useNearby";
import { NearbyPanel } from "@/components/Discovery/NearbyPanel";
import { ExplorationLogs } from "@/components/Exploration/ExplorationLogs";
import { ExploreLogModal } from "@/components/Exploration/ExploreLogModal";
...
const panelVariants = {
  ...
  visible: { x: 0, opacity: 1, transition: { type: "spring" as const, damping: 34, stiffness: 240, mass: 0.9 } },
  exit:    { x: "105%", opacity: 0, transition: { duration: 0.22, ease: [0.4, 0, 1, 1] as const } },
};
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-1.5">
          ...
          <span style={{ fontSize: "11.5px", color: "rgba(255,255,255,0.32)", fontFamily: FONT }}>Urban Decay Score</span>
        </div>
        <span className="font-sans font-bold tabular-nums" style={{ fontSize: "18px", color, fontFamily: DISPLAY_FONT, letterSpacing: "-0.02em" }}>
          {score}
        </span>
      </div>
...
function RankLockOverlay({ tier }: { tier: string }) {
  return (
    ...
    >
      ...
      >
        <div className="w-16 h-16 rounded-3xl flex items-center justify-center" style={{ background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.2)" }}>
          <Lock className="w-6 h-6" style={{ color: "#f43f5e" }} />
        </div>
        <div>
          <p className="font-sans font-bold text-white mb-2" style={{ fontSize: "18px", fontFamily: DISPLAY_FONT, letterSpacing: "-0.03em" }}>Extreme Zone</p>
          ...
          <p className="font-sans mt-1.5" style={{ fontSize: "11px", color: "rgba(255,255,255,0.2)", fontFamily: FONT }}>Current: {tier}</p>
        </div>
      </motion.div>
    </motion.div>
  );
...
function AdminControls({
  locationId, currentVerification, onRemoved, onVerificationChanged,
}: {
  locationId:             string;
  ...
  async function handleVerify(state: VerificationState) {
    setBusy(true);
    try { await updateVerificationState(locationId, state); onVerificationChanged(state); }
  ...
  async function handleDelete() {
    ...
    setBusy(true);
    try { await deleteLocation(locationId); onRemoved(); }
    finally { setBusy(false); }
  }
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ background: "rgba(245,158,11,0.04)", border: "1px solid rgba(245,158,11,0.12)" }}>
          ...
          {STATES.map(({ state, label, icon, color }) => {
            ...
            return (
              <button
                ...
                style={{
                  fontSize: "11px", fontFamily: FONT, fontWeight: isActive ? 600 : 400,
...
export function LocationPanel({
  location, onClose, onSelectLocation, allLocations,
  userRankTier = "Scout", isAdmin = false, onLogAdded,
}: LocationPanelProps) {
  ...
  const locId                                        = displayedLocation ? String(displayedLocation.id) : null;
  const { logs, loading: logsLoading, addLog, deleteLog } = useExplorationLogs(locId);
  ...
  const locIdStr     = displayedLocation ? String(displayedLocation.id) : "";
  const isSaved      = savedIds.has(locIdStr);
  const isExplored   = exploredIds.has(locIdStr);
  ...
  const rarity    = displayedLocation ? getLocationRarity(displayedLocation.riskLevel, displayedLocation.abandonmentScore) : null;
  const rarMeta   = rarity ? RARITY_META[rarity] : null;

  const handleSave = useCallback(async () => {
    if (!user) { navigate("/login"); return; }
    await toggleSave(locIdStr);
  }, [user, locIdStr, toggleSave, navigate]);

  const handleExplore = useCallback(async () => {
    if (!user) { navigate("/login"); return; }
    await toggleExplore(locIdStr);
  }, [user, locIdStr, toggleExplore, navigate]);
    ...
    <>
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className="flex items-center gap-2 mb-3 flex-wrap">
                      ...
                      {rarMeta && (
                        <>
                          <span style={{ color: "rgba(255,255,255,0.12)", fontSize: "12px" }}>·</span>
                          <span
                            className="font-sans font-semibold px-1.5 py-0.5 rounded-full"
                            style={{ fontSize: "10px", color: rarMeta.color, background: `${rarMeta.color}12`, border: `1px solid ${rarMeta.color}25` }}
                          >
                            {rarMeta.label}
                          </span>
                        </>
                      ...
                      {isFresh && (
                        <motion.span
                          ...
                          style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", fontSize: "10px", color: "rgba(255,255,255,0.5)", fontFamily: FONT, fontWeight: 600 }}
                        >
                  ...
                  {!isLocked && (
                    <div className="px-7 py-5 flex-shrink-0 space-y-4" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                      ...
                      <motion.button
                        ...
                        className="w-full flex items-center justify-center gap-2.5 py-3 rounded-2xl font-sans font-semibold"
                        style={{
                          fontSize:   "13.5px",
                          fontFamily: FONT,
      ...
      {displayedLocation && (
        <ExploreLogModal
          ...
          onSave={async (data) => {
            await addLog(String(displayedLocation.id), data);
            onLogAdded?.();
          }}
        />
      )}
